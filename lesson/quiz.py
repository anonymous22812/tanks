print("Дед это-")
print('а) ИС-4')
print('б) Hetzer')
print('в) ИС-7')
print('г) M-4 Sherman')
print('д) т-34')

answer1 = input('Ответ: ')

if answer1 == 'в':
    print(' Ты норм танкист')
elif answer1 == 'а':
    print("Иди в танки поиграй!")
elif answer1 == 'б':
    print("Иди в танки поиграй!")
elif answer1 == 'д':
    print("Иди в танки поиграй!")
elif answer1 == 'г':
    print("Иди в танки поиграй!")

print('Бабаха это-')
print('а) AMX 13 75')
print('б) КВ-2')
print('в) Grille 15')
print('г) Churchill 1')
print('д) FV 215b (183)')


answer2 = input('Ответ: ')


if answer2 == 'д':
    print(' Ты норм танкист')
elif answer2 == 'а':
    print("Иди в танки поиграй!")
elif answer2 == 'б':
    print("Иди в танки поиграй!")
elif answer2 == 'в':
    print("Иди в танки поиграй!")
elif answer2 == 'г':
    print("Иди в танки поиграй!")

print('Гриль это-')
print('а) Grille 15')
print('б) КВ-1С')
print('в) ЛТГ')
print('г) АТ-2')
print('д) M5 Stuart')

answer3 = input('Ответ: ')

if answer3 == 'а':
    print(' Ты норм танкист')
elif answer3 == 'д':
    print("Иди в танки поиграй!")
elif answer3 == 'б':
    print("Иди в танки поиграй!")
elif answer3 == 'в':
    print("Иди в танки поиграй!")
elif answer3 == 'г':
    print("Иди в танки поиграй!")


print('Ветка боли это-')
print('а) ветка ИС-7')
print('б) ветка гриля')
print('в) ветка бабахи')
print('г) ветка мыши')
print('д) ветка сосисы')


answer4 = input('Ответ: ')

if answer4 == 'в':
    print(' Ты хороший танкист')
elif answer4 == 'д':
    print("")
elif answer4 == 'б':
    print("Иди в танки поиграй!")
elif answer4 == 'а':
    print("Иди в танки поиграй!")
elif answer4 == 'г':
    print("Иди в танки поиграй!")

print('Китайская пародия это-')
print('а) т-34')
print('б) type t-34')

answer5 = input('Ответ: ')

if answer5 == 'б':
    print(' Ты хороший танкист')
elif answer5 == 'а':
    print("Как ты так мог сказать про легенду?!!")


print('T-95 это-')
print('а) черепаха')
print('б) яга')
print('в) борщ')
print('г) бревномет')
print('д) елка')

answer6 = input('Ответ: ')

if answer6 == 'а':
    print(' Ты хороший танкист')
elif answer6 == 'д':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'в':
    print("Иди в танки поиграй!")
elif answer6 == 'г':
    print("Иди в танки поиграй!")

print('ARL-44 это-')
print('а) VI уровень')
print('б) V уровень')
print('в) III уровень')
print('г) X уровень')
print('д) VIII уровень')

answer6 = input('Ответ: ')

if answer6 == 'а':
    print(' Ты хороший танкист')
elif answer6 == 'д':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'в':
    print("Иди в танки поиграй!")
elif answer6 == 'г':
    print("Иди в танки поиграй!")

print('Маus это-')
print('а) IV уровень')
print('б) V уровень')
print('в) I уровень')
print('г) X уровень')
print('д) VII уровень')

answer6 = input('Ответ: ')

if answer6 == 'г':
    print(' Ты хороший танкист')
elif answer6 == 'д':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'в':
    print("Иди в танки поиграй!")
elif answer6 == 'а':
    print("Иди в танки поиграй!")

print('Tortoise это-')
print('а) IV уровень')
print('б) VI уровень')
print('в) IX уровень')
print('г) III уровень')
print('д) VII уровень')

answer6 = input('Ответ: ')

if answer6 == 'В':
    print(' Ты хороший танкист')
elif answer6 == 'д':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'Г':
    print("Иди в танки поиграй!")
elif answer6 == 'а':
    print("Иди в танки поиграй!")

print('СУ-85Б это-')
print('а) I уровень')
print('б) II уровень')
print('в) III уровень')
print('г) IV уровень')
print('д) V уровень')

answer6 = input('Ответ: ')

if answer6 == 'г':
    print(' Ты хороший танкист')
elif answer6 == 'д':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'в':
    print("Иди в танки поиграй!")
elif answer6 == 'а':
    print("Иди в танки поиграй!")

print('Крушитель это-')
print('а) IV уровень')
print('б) II уровень')
print('в) V уровень')
print('г) VIII уровень')
print('д) VII уровень')

answer6 = input('Ответ: ')

if answer6 == 'д':
    print(' Ты хороший танкист')
elif answer6 == 'г':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'в':
    print("Иди в танки поиграй!")
elif answer6 == 'а':
    print("Иди в танки поиграй!")

print('Какая самая последняя добавленая прокачеваемая десятка?')
print('а) Foch 155')
print('б) WZ-113b')
print('в) Sheridan')
print('г) Minotauro')
print('д) Vickers Light')

answer6 = input('Ответ: ')

if answer6 == 'г':
    print(' Ты хороший танкист')
elif answer6 == 'д':
    print("Иди в танки поиграй!")
elif answer6 == 'б':
    print("Иди в танки поиграй!")
elif answer6 == 'в':
    print("Иди в танки поиграй!")
elif answer6 == 'а':
    print("Иди в танки поиграй!")


print('Твоя любимая десятка')
print('а) T110E4')
print('б) T110E3')
print('в) T110E5')
print('г) M-VI-Yoh')
print('д) M48 Patton')
print('е) T57 Heavy')
print('ё) Sheridan')
print('ж) Grille 15')
print('з) Jg.Pz. E 100')
print('и) VK 72.01 K')
print('й) Maus')
print('к) E 100')
print('л) E 50 M')
print('м) Leopard 1')
print('н) Об. 268')
print('о) Об. 263')
print('п) ИС-7')
print('р) ИС-4')
print('с) Об. 140')
print('т) Т-62А')
print('у) Т-100 ЛТ')
print('ф) FV215b 183')
print('х) FV4005')
print('ц) FV4002')
print('ч) Vickers Light')
print('ш) FV215b')
print('щ) Ho-Ri')
print('э) STB-1')
print('ю) Type 71')
print('я) В данном списке такого нет. Дальше ==>')

answer6 = input('Ответ: ')

if answer6 == 'а':
    print('Круто, моя Маус.')
elif answer6 == 'б':
    print("Круто, моя Маус.")
elif answer6 == 'в':
    print("Круто, моя Маус.")
elif answer6 == 'г':
    print("У него крутая механика. А моя Маус!")
elif answer6 == 'д':
    print("Круто, моя Маус.")
elif answer6 == 'е':
        print('Круто, моя Маус.')
elif answer6 == 'ё':
        print("Базука вместо пушки? Круто, моя Маус.")
elif answer6 == 'ж':
        print("Самый меткий ствол в игре. Круто, моя Маус.")
elif answer6 == 'з':
        print("Идеальное сочетание брони и хорошего орудия в оболочке ПТ-САУ. Круто, моя Маус.")
elif answer6 == 'и':
        print("Тапок? Круто, мой Маус!")
elif answer6 == 'й':
    print('Круто, я тоже его обожаю!')
elif answer6 == 'к':
    print("Ты знал(а) что он должен был весить 130 тонн? А еще от него собрали только ходовую в реальной жизни. Круто, моя Маус.")
elif answer6 == 'л':
    print("Я слышал что он имба. Круто! Мой Маус.")
elif answer6 == 'м':
    print("Немецкий лт. Я слышал он скилозависимый, надеюсь ты статист. Круто, моя Маус.")
elif answer6 == 'н':
    print("Ветка бревнометов просто имба (у самого 2 сушки есть). Круто, моя Маус.")
elif answer6 == 'о':
        print("Я слышал что он имба. Круто! Мой Маус.")
elif answer6 == 'п':
        print("Я не удивлен) Круто! Мой Маус.")
elif answer6 == 'р':
        print("Жалко что нанем уже мало кто играет. Круто, моя Маус.")
elif answer6 == 'с':
        print("Круто, моя Маус.")
elif answer6 == 'т':
    print('Круто, моя Маус.')
elif answer6 == 'у':
    print("Я не удивлен) Круто! Моя Маус.")
elif answer6 == 'ф':
    print("Я не удивлен) Круто! Мой Маус.")
elif answer6 == 'х':
    print("Я не удивлен) Круто! Моя Маус.")
elif answer6 == 'ц':
    print("Круто, моя Маус!")
elif answer6 == 'ч':
    print("Круто, моя Маус!")
elif answer6 == 'ш':
    print("Круто! Моя Маус.")
elif answer6 == 'щ':
    print("Круто, моя Маус.")
elif answer6 == 'э':
    print("Круто, моя Маус.")
elif answer6 == 'ю':
    print("Слышал, он имба. Круто, моя Маус.")
elif answer6 == 'я':
    print('Вот остальные')


print('Твоя любимая десятка')
print('а) WZ-121')
print('б) WZ-113G FT')
print('в) WZ-113')
print('г) Foch 155')
print('д) AMX 50 b')
print('е) B-C 25t')
print('ё) Minotauro')
print('ж) Progetto 65')
print('з) Kranvagn')
print('и) 60TP Levandovskiego')
print('й) TVP T 50/51')
print('к) Я выбрал(а) в прошлый раз')

if answer6 == 'а':
    print('Круто, моя Маус.')
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'б':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'в':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'г':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'д':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'е':
    print('Круто, моя Маус.')
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'ё':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'ж':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'з':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'и':
    print("Круто, моя Маус.")
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'й':
    print('Круто, моя Маус.')
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")
elif answer6 == 'к':
    print('Окей, круто')
    print("Спасибо что прошел этот тест, мне очень приятно, я старался")
    print("Удачи тебе")



